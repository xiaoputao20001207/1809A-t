export default {
    //头部
    'header.training':'Training',//实训
    'header.training-plan':'plan',//计划
    'header.training-progress':'progress',//进度
    'header.training-plea':'plea',//答辩

    'header.interview':'Interview',//面试
    'header.interview-interview Record Management':'Interview Record Management',//面试管理记录
    'header.interview-interview leaderboard':'Interview leaderboard',//面试排行榜
    'header.interview-the interview records':'the interview records',//面试记录
    
    'header.question':'Question',//问答
    'header.question-list':'Question list',//问答列表
    'header.question-management':'Question Management',//问答管理

    'header.project':'Project',//项目
    'header.jobs':'Jobs',//岗位

    //脚部
    'footer.into the eight dimensions':'Into the eight dimensions',//走进八维
    'footer.eight dimensions culture':'Eight dimensions culture',//八维文华
    'footer.the social responsibility':'The social responsibility',//社会责任
    'footer.contact us':'Contact us',//联系我们
    // 'footer.':'',
    // 'footer.':'',
    // 'footer.':'',
    // 'footer.':'',
    // 'footer.':'',
    // 'footer.':'',
}